...
class Backdrop:
    ...
    def __init__(
        self, SCREEN_WIDTH, SCREEN_HEIGHT, image_background, image_base, scale
    ):
        self.SCREEN_WIDTH = SCREEN_WIDTH
        self.SCREEN_HEIGHT = SCREEN_HEIGHT
...
class FlappyBird(base.PyGameWrapper):
    ...
    def __init__(self, width=288, height=512, pipe_gap=100):
        ...
        self.init_pos = (int(self.width * 0.2), int(self.height / 2))
    ...
    def _load_images(self):
        ...
        for c in ["red", "blue", "yellow"]:
            ...
            self.images["player"][c] = [
                pygame.image.load(im).convert_alpha() for im in image_assets
            ]
    ...
    def getGameState(self):
        ...
        state = {
            ...
            "next_pipe_dist_to_player": next_pipe.x
            + next_pipe.width / 2
            - self.player.pos_x,
    ...
    def _generatePipes(self, offset=0, pipe=None):
        start_gap = self.rng.randint(self.pipe_min, self.pipe_max)
    ...
    def step(self, dt):
        ...
        for p in self.pipe_group:
            ...
            is_in_pipe = (
                (p.x - p.width / 2 - 20) <= self.player.pos_x < (p.x + p.width / 2)
            )
            for h in hit:  # do check to see if its within the gap.
                top_pipe_check = (
                    (self.player.pos_y - self.player.height / 2 + 12) <= h.gap_start
                ) and is_in_pipe
                bot_pipe_check = (
                    (self.player.pos_y + self.player.height)
                    > h.gap_start + self.pipe_gap
                ) and is_in_pipe
